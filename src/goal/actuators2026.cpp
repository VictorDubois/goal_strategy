#include "../include/goal_strategy/actuators2026.h"
#include <unistd.h> //usleep

Actuators2026::Actuators2026(rclcpp::Node::SharedPtr a_node,
                             std::string a_name,
  ...
  , m_pump_4(a_pump_4)
{
    m_pub = m_node->create_publisher<krabi_msgs::msg::Actuators2026>(a_name, 5);
    m_shutdown = false;
}

void Actuators2026::updateStepperElevator(krabi_msgs::msg::InfosStepper::SharedPtr stepper_info_msg)
{
    m_stepper_elevator_1->updateElevatorInfos(stepper_info_msg);
}

void Actuators2026::start()
{
    m_running = std::thread(&Actuators2026::run, this);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "actuators 2025 running");
}

void Actuators2026::run()
{
    while (true)
    {
        usleep(100000);
...
void Actuators2026::set_score(int a_score)
{
    m_score = a_score;
}

void Actuators2026::shutdown()
{
    m_shutdown = true;
}

void Actuators2026::publish()
{
